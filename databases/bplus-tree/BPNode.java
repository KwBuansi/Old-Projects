...
public class BPNode<K extends Comparable<K>, V> {
	...
	public void insertChild(K key, int childNumber, BPNodeFactory<K,V> nodeFactory) {
		...
		//nodeFactory.save(child); // Will work later after you implement save()
	}
	...
	public SplitResult<K,V> splitLeaf(BPNodeFactory<K,V> nodeFactory) {
		...
		// TODO ...
		for(int i = middleIndex; i < result.left.keys.size(); ++i){
			result.right.keys.add(result.left.keys.get(i)); // move the key(i) to the right side
			result.left.keys.remove(i); // remove it from the left side
			result.right.values.add(result.left.values.get(i)); // move the value(i) to the right side
			result.left.values.remove(i); // remove it from the left side
		}
		result.dividerKey = result.right.getKey(0); // divider is the first key of the right node
	...
	public SplitResult<K, V> splitInternal(BPNodeFactory<K,V> nodeFactory) {
		...
		result.left = this;
		result.right = nodeFactory.create(true);
		int middleIndex = (keys.size() + 1)/2;
		result.dividerKey = keys.get(middleIndex);
		for(int i = middleIndex; i < result.left.keys.size(); ++i){
			result.right.keys.add(result.left.keys.get(i)); // move the key(i) to the right side
			result.left.keys.remove(i); // remove it from the left side
			result.right.children.add(result.left.children.get(i)); //move the child(i) to the right side
			result.left.children.remove(i); // remove it from the left side
		}
		result.left.parent = this.parent;
	...
	public void load(ByteBuffer buffer, Function<String, K> loadKey, Function<String, V> loadValue) {
		...
		int type = buffer.getInt(); // 1 is leaf, 0 is internal
		
		int representationSize = buffer.getInt();
		...
		if(/* type represents a leaf */) {
			// String representation = read from the disk representation
			// String[] fields = representation.split("#")

			// Loading keys
			// for(i: 0 -> number_keys)
			//   add to keys array loadKey.apply(fields[i])

			// Loading values
			// ...
		}
		else {
			// Load as X keys, X + 1 integers

			// Loading keys
			//...

			// Load children integers
		}
	}
