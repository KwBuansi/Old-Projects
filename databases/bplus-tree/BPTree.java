...
public class BPTree<K extends Comparable<K>, V> {
	...
	public void insert(K key, V value) {
		...
		if(insertPlace.keys.size() == BPNode.SIZE) {
			...
			// You have result.left, result.right, perform insertOnParent()
	...
	private void insertOnParent(BPNode<K,V> left, K key, BPNode<K,V> right) {
		...
		if(left.number == rootNumber) {
			// Create a new root
			BPNode<K,V> root = nodeFactory.create(false);
			rootNumber = root.number;
			// link the root to children
			root.children.add(left.number);
			root.children.add(right.number);
			left.parent = rootNumber;
			right.parent = rootNumber;
		}
		else {
			// BPNode, ask for the left node's parent
			...
			if(parent.keys.size() == BPNode.SIZE) {
				// Need to split the node 
				// Call parent.splitINTERNAL();	
				SplitResult<K, V> result =  parent.splitInternal(nodeFactory);
				result.left.parent = left.parent;
				result.right.parent = left.parent;
				// You have result.left, result.right, perform insertOnParent()
				// Need to call insertOnParent after performing a leaf node split
				// Call insertOnParent recursively (left, dividerKey, right);
				insertOnParent(result.left, result.dividerKey, result.right);
			}
		}
	}
	 ...
	 */
	public V get(K key) {
		// find() is your helper method
		BPNode<K,V> findNode = find(nodeFactory.getNode(rootNumber), key);
		for (int i = 0; i < findNode.keys.size(); i++) {
			if (BPTree.equal(findNode.getKey(i), key) ){
				return findNode.getValue(i);
			}
		}
		return null;
	}
	...
	private BPNode<K,V> find(BPNode<K,V> node, K key) {
		// Do not use recursion, but iteration
		// If you use recursion, there's a chance one of your current nodes in the stack gets evicted
		// from the buffer cache when you unwind the recursion

		// Compare using more() like this:
		// 	if(more(node.getKey(i), key)) ...
		BPNode<K,V> currentNode = node;

		while (!currentNode.isLeaf()){
			int i = 0;
			while (i < currentNode.keys.size() && BPTree.less(currentNode.getKey(i), key)){ //go to the smallest index >= key
				++i;
			}
			if (i < currentNode.keys.size() && BPTree.equal(currentNode.getKey(i), key)){ //is it equal to value?
				currentNode = nodeFactory.getNode(currentNode.getChild(i + 1)); //then get the child to the right of it
			} else if (i == currentNode.keys.size()){ //is it the last index?
				currentNode = nodeFactory.getNode(currentNode.getChild(currentNode.keys.size())); //then go to the last index's child
			} else {
				currentNode = nodeFactory.getNode(currentNode.getChild(i)); //otherwise go to the left child
			}
		}
